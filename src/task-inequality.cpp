...
namespace dynamicgraph
{
  namespace sot
  {
	...
	const dynamicgraph::Vector & error = errorSOUT(time);
	const dynamicgraph::Vector & refInf = withInf ? referenceInfSIN(time) : dummy;
	const dynamicgraph::Vector & refSup = withSup ? referenceSupSIN(time) : dummy;
	const Flags & selec = selecSIN(time);
	...
	return res;
      ...
      dynamicgraph::Matrix& TaskInequality::
      computeJacobian( dynamicgraph::Matrix& res,int time )
      {
	const Flags & selec = selecSIN(time);
	dynamicgraph::Matrix Jin;  Task::computeJacobian(Jin,time);
	...
	return res;
      ...
      dynamicgraph::Vector& TaskInequality::
