...
namespace dynamicgraph
{
  namespace sot
  {
	...
	const dynamicgraph::Vector & error = errorSOUT(iter);
	const dynamicgraph::Vector & errorDot = errorDotSOUT(iter);
	const dynamicgraph::Vector & refInf = withInf ? referenceInfSIN(iter) : dummy;
	const dynamicgraph::Vector & refSup = withSup ? referenceSupSIN(iter) : dummy;
	...
	return res;
      ...
      dynamicgraph::Matrix& TaskDynInequality::
      computeJacobian( dynamicgraph::Matrix& res,int iter )
      {
	const Flags & selec = selecSIN(iter);
	dynamicgraph::Matrix Jin;  TaskDynPD::computeJacobian(Jin,iter);
